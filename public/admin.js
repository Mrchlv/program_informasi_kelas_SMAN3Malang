// Variabel untuk menyimpan data siswa (contoh data)
let students = [
...
[
  ...
  function renderStudentTable() {
    const tableBody = document.getElementById('admin-student-list');
    tableBody.innerHTML = '';
    ...
    students.forEach(student => {
      const row = `
        <tr>
          ...
          <td>${student.kode}</td>
          <td>${student.kelas}</td>
          <td><button class="delete-button" data-kode="${student.kode}">X</button></td>
        </tr>
      `;
      tableBody.innerHTML += row;
    });
  
    // Menambahkan event listener untuk tombol hapus pada setiap baris tabel siswa
    const deleteButtons = document.querySelectorAll('.delete-button');
    deleteButtons.forEach(button => {
      button.addEventListener('click', () => {
        const kode = button.dataset.kode; // Mendapatkan kode siswa dari atribut data
        deleteStudent(kode); // Memanggil fungsi deleteStudent untuk menghapus siswa
      });
    });
  }
  
  // Fungsi untuk menghapus data siswa berdasarkan kode
  function deleteStudent(kode) {
    students = students.filter(student => student.kode !== kode);
    renderStudentTable(); // Memuat ulang tabel setelah menghapus data
  }
  ...
  adminForm.addEventListener('submit', function(event) {
    ...
    const kode = document.getElementById('admin-student-code').value;
    const nama = document.getElementById('admin-student-name').value;
    const kelas = document.getElementById('admin-student-class').value;
  
    // Menambahkan siswa baru ke dalam array students
    students.push({ nama, kode, kelas });
  
    // Memanggil fungsi untuk merender kembali tabel siswa
    renderStudentTable();
  ...
  // Event listener untuk membatasi jumlah baris tabel
  const tableLimit = document.getElementById('table-limit');
  tableLimit.addEventListener('change', function() {
    renderStudentTable(); // Memuat ulang tabel saat perubahan limit baris
  });
  
  // Event listener untuk mengurutkan siswa berdasarkan kelas
  const sortByClass = document.getElementById('sort-by-class');
  sortByClass.addEventListener('change', function() {
    renderStudentTable(); // Memuat ulang tabel saat perubahan opsi sortir
  });
  
  // Memanggil fungsi untuk merender tabel pada saat halaman dimuat
